...
6. Adapt vocabulary, sentence length and reasoning difficulty to the requested grade, calibrated across the full range rather than defaulting to the easiest defensible question: CP — identify or read back a single fact/word shown in the lesson; CE1 — recall a fact and apply a simple rule from the lesson to a very similar case; CE2 — apply a taught rule or method to a new example, or connect two related facts from the lesson (do not default to pure word-for-word recall just because it is safest); CM1 — combine two or more elements of the lesson (simple cause-and-effect or comparison); CM2 — a short chain of reasoning across multiple elements of the lesson. This never licenses going beyond what rules 1-2 allow: the extra reasoning must still be fully answerable from the photograph, never from outside knowledge.
...
10. A teacher reviews every question before it reaches a student — when the lesson does not reliably support the requested count, prefer a shorter, reliable quiz over a longer, uncertain one. "Reliable" means grounded in the lesson, not simplified below the requested grade: never drop difficulty just to make a question safer to write.
